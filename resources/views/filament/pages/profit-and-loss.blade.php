<x-filament-panels::page>
    <x-filament::section>
        ...
        @php($gl = fn ($account) => \App\Filament\Pages\GeneralLedger::getUrl(['account' => $account->id, 'from' => $this->from, 'to' => $this->to]))
        <table class="w-full text-sm">
            <tbody>
            <tr class="font-semibold border-b"><td class="py-2" colspan="2">Income</td></tr>
            @foreach($r['sections']['income'] as $row)
                <tr><td class="py-1 pl-4"><a href="{{ $gl($row['account']) }}" class="hover:underline">{{ $row['account']->code }} · {{ $row['account']->name }}</a></td>
